...
export async function POST(request: Request, { params }: { params: Promise<{ id: string }> }) {
  ...
  try {
    ...
    if (orderError) {
      console.warn(`[receipt] orders select error for ${id}:`, orderError.message)
    ...
    if (uploadError) {
      console.warn(`[receipt] storage upload error for ${id}:`, uploadError.message)
    ...
    // Catch-all para excepciones no manejadas (PostgREST throw, storage throw, etc.).
    // Devolvemos 500 con el mensaje en el cuerpo para debugging, y dejamos traza en logs.
    const message = err instanceof Error ? err.message : "Error inesperado"
    console.error(`[receipt] uncaught error for ${id}:`, err)
